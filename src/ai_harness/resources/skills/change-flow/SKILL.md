---
name: change-flow
description: "Change orchestrator skill — coordinates file-backed Changes through the explore → prd → design → specs → tasks → implement → validate → archive pipeline. Use for managed-change triggers like 'do this as a change' or 'use change flow'."
license: Apache-2.0
metadata:
  author: diegoagd10
  version: "1.0"
...
# Change Flow (Orchestrator)

You are a COORDINATOR, not an executor. Keep one thin conversation thread,
delegate heavy work to sub-agents, synthesize results. This skill is the
skill-form of the `change-orchestrator` agent; each pipeline phase has its
own `change-*` skill (`change-explorer`, `change-propose`, `change-design`,
`change-specs`, `change-tasks`, `change-implementor`, `change-validator`,
`change-archiver`) that you load or inject when running that phase.
...
- git commands (`status`, `diff`, `log`, `add`, `commit`) run inline via
  `bash`.
- NEVER spawn a sub-agent for class 1 or class 2 work.
...
Then delegate IMMEDIATELY to ONE general-purpose sub-agent. Hard rules:
...
- Memory lookups never replace doing the work — delegate first, save
  discoveries after.
...
one"), invoke that skill FIRST and follow its instructions for your reply.
Name the skill in your reply (e.g. "Applying grill-me-one-by-one").
Skill-driven interviews are conversation, not execution — they never
trigger delegation or change flow by themselves.
...
Before `change-new`, run a quick similarity check: a memory search
(read-only) plus a look at `.ai-harness/changes/` and
`.ai-harness/archive/`. Active folder match → recommend `change-continue`
and wait. Archived match → default stop; offer a new name. Stale memory
with no folder → proceed. No match → proceed. Never auto-route; on a
match the user decides.
...
`{"errors": [...]}` on failure — run by the archiver phase, never
by you.
...
and never dispatches a phase worker when `nextRecommended` is
`resolve-blockers`.

### Forward `configContext` to the phase worker

When `change-continue` returns an actionable `nextRecommended`, the
returned `configContext` is the canonical phase ruleset the next phase
worker needs in order to do its job. Forward that JSON object to the
phase worker — verbatim, with no rewrite, no independent
...
anything and do not run a phase.
...
Do not run any phase in the same turn. A generic "continue" is
...
After the answers, summarize the resulting assumptions before running
PRD.
...
After every phase completes, rerun
`ai-harness change-continue {change}` and route on its
`nextRecommended`:

| `nextRecommended` | Phase skill to load / inject |
| --- | --- |
...
Phase result blocks are completion signals, not routing decisions.
...
run the same phase for the same change twice in one session unless the
...
1. The next `change-continue` still routes to `prd` as usual — run the
   `change-propose` phase, but set its concrete goal to an OVERVIEW prd:
...
2. Once the parent's `prd.md` is written, STOP advancing the parent's
   own pipeline. Never run `design`, `specs`, `tasks`, `implement`, or
   `validate` for the parent, and do not rerun
   `change-continue {parent}` afterward — its `prd.md` is shared
   reference context for the children, not an implementation target.
3. Process each confirmed child in turn through the normal Class 4
   ...
   the child's `explore` and `prd` phases can read it for high-level
   scope context before writing the child's own, narrower `prd.md`.

**Human review gate.** Before running `implement`, list the artifact
paths the implementor will read (`exploration.md`, `design.md` — see
"Phase input contracts" below) and require an explicit
...
**Archive.** Run the `change-archiver` phase; it runs
...
Phase workers start with NO memory beyond what you give them. Every phase
invocation MUST include: the change name and root
(`.ai-harness/changes/{change}/`), the concrete goal, and the persona
contract. Add exactly the per-phase data in "Phase input contracts"
below — never more, never less. When delegating to a sub-agent, pass exact
`SKILL.md` paths under a `Skills to load before work:` block — never
summaries, never invented paths. The implementor phase always gets the
TDD skill path when it resolves; if a skill cannot be resolved, say so
and continue (`skills: fallback`).

## Phase input contracts (HARD GATE)

Before every phase, check this table and pass exactly what it lists —
no more, no less. "Inject" is data the phase worker has no other way to
get (missing it is a missing input, not a gap the worker can fill in).
"Reads on its own" is what the worker opens from disk or the CLI once it
has the change root — do NOT paste those contents into the prompt.

| Phase | Purpose | Inject (beyond change name/root + persona) | Reads on its own |
| --- | --- | --- | --- |
...
fork — budget"), inject one more item into the explore and propose
scope seeds: the parent's PRD path
(`.ai-harness/changes/{parent}/prd.md`). This is additive to the
"shared understanding / scope seed" row above, not a replacement, and
applies to no other phase.

## Implementor delegation data (HARD GATE)

The implementor phase refuses to commit without a `commit-format:`
directive, so every implementor invocation MUST carry one. Before
running it:
...
3. If `configContext` is `null` or has no `commit_format` value when
   ...
   the normal flow): do NOT run the implementor phase. Report
