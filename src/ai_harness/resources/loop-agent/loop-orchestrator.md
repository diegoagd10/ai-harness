...
- Current branch on the host is `main`, working tree clean — OR you are inside a detached worktree at `main`'s HEAD (created via `ai-harness worktree create`).
...
8. **Validate-and-fix loop.** Delegate to `validator`. Pass: issue number, title, body, the exact sub-branch string from step 3 (`loop/<issue-number>-<Date.now()>`), `base_branch=<loop_run_branch>`, and `prd_ref`. Validator reads `git diff <loop_run_branch>...<sub-branch>` (this isolates just this issue's change, even though `<loop_run_branch>` already has earlier issues' commits on it), runs the project's quality gates from `CODING_STANDARDS.md`, and runs the user-stories coverage check.
   ...
   - **Arbitrate non-reproducing gate failures.** If the implementor returns `GATE-NOT-REPRODUCED`, or the only remaining finding is a gate FAIL the implementor can't reproduce: run that gate yourself on the sub-branch's clean HEAD (`git stash -u` any local junk first, since a dirty working tree is the usual cause of a phantom gate FAIL). If it passes, treat the issue as clean and proceed to step 9 — do not burn another fix-up round. If it genuinely fails, continue the fix-up loop.
