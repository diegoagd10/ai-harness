"""Resolver: change selection, artifact classification, state-machine gates.

Ported from cli.bak/tests/test_resolver.py with these adaptations:
- applyProgress → applyReport, apply-progress.md → apply-report.md
- resolve() signature drops include_instructions parameter
- Instructions-related tests removed (out of scope)
"""
...
def _resolve(root: Path, change: str = "thin"):
    return resolve(str(root), "", change)
...
def test_explicit_missing_change_blocks_with_sdd_new(tmp_path: Path):
    ...
    status = resolve(str(tmp_path), "", "missing")
...
def test_artifact_states_and_task_progress(tmp_path: Path):
    ...
    write_file(change_root / "apply-report.md", "# Apply\n")

    status = resolve(str(tmp_path), "", "add-auth")
    ...
    assert status.artifacts["applyReport"] == "done"
    ...
    assert status.dependencies.verify == "ready"  # apply-report present
...
def _seed_apply_report(root: Path) -> None:
