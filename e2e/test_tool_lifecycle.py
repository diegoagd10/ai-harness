...
def _assert_binary_on_path(bin_dir: str) -> None:
    ...
    if found is None:
        raise AssertionError(f"ai-harness not found on PATH after tool install (bin_dir={bin_dir})")
...
def _assert_binary_not_on_path(bin_dir: str) -> None:
    ...
    if found is not None:
        raise AssertionError(f"ai-harness still on PATH after tool uninstall (bin_dir={bin_dir}, found={found})")
