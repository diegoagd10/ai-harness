...
4. **Verify Code Quality** - run the following commands to verify code quality, if any issues found, then fix them:
  - `uv run ruff format --check .`
  - `uv run ruff check .`
  - `uv run pylint --disable=all --enable=duplicate-code --recursive=y ./src`
  - `uv run pylint --disable=all --enable=duplicate-code --recursive=y ./tests`
  - `uv run pylint --disable=all --enable=duplicate-code --recursive=y ./e2e`
4. **Verify** — run `uv run pytest` and `./e2e/docker-test.sh` before committing. Fix any failures before proceeding.
